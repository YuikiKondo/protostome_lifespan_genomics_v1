...
## RUN PGLS (mono-nucleotide rates as response; additive model) ----
...
for (region in regions) {
  ...
  for (trait in traits) {
    ...
    cat("  Running additive PGLS (", trait, " as response)\n", sep = "")
...
if (length(pgls_results_all) > 0) {
  ...
  out_file <- "pgls_results_6_gene_associated_regions_mononuc_rates_LS_CENTERED_G_CENTERED_no_interaction_mono_as_response.csv"
