...
## LOAD DATA ----
input_file <- "di_rates_rev_comp_collapsed_genome_and_intergenic_with_lifespan_tip_names_and_genome_size_Github_check.csv"
...
## ---- Define which dinuc RATE columns to use (collapsed for genome + intergenic) ----
collapsed_regions <- c("genome", "intergenic")

# Non-collapsed (16) dinucleotide RATE columns for other regions
dinuc_rate_16 <- grep("_Rate$", colnames(matched_data), value = TRUE)
# Remove mono-nucleotide rate columns if present
dinuc_rate_16 <- setdiff(dinuc_rate_16, c("A_Rate", "T_Rate", "G_Rate", "C_Rate"))
# Remove collapsed RATE columns from the 16-set
dinuc_rate_16 <- dinuc_rate_16[!grepl("_and_", dinuc_rate_16)]
dinuc_rate_16 <- sort(dinuc_rate_16)

# Collapsed (10) RATE metrics for genome + intergenic:
# 6 paired collapsed + 4 palindromes (kept as-is)
dinuc_rate_collapsed_10 <- c(
  grep("_and_.*_Rate$", colnames(matched_data), value = TRUE),
  "AT_Rate", "TA_Rate", "CG_Rate", "GC_Rate"
) %>% unique()

dinuc_rate_collapsed_10 <- dinuc_rate_collapsed_10[dinuc_rate_collapsed_10 %in% colnames(matched_data)]
dinuc_rate_collapsed_10 <- sort(dinuc_rate_collapsed_10)

## RUN PGLS WITH INTERACTION TERMS (RAW RATES AS RESPONSE) ----
pgls_results_all_regions <- list()

for (region in regions) {
  cat("Running PGLS (raw rates as response) for region:", region, "\n")
  ...
  dinuc_columns_to_use <- if (region %in% collapsed_regions) dinuc_rate_collapsed_10 else dinuc_rate_16

  if (length(dinuc_columns_to_use) == 0) {
    message("No dinucleotide RATE columns found for region: ", region)
    next
  }
  ...
  for (dinuc in dinuc_columns_to_use) {
    ...
    region_df <- region_data %>%
      transmute(
        ...
        rate_raw         = suppressWarnings(as.numeric(.data[[dinuc]]))
      ) %>%
      # Keep finite values; rates can be zero, so do NOT log-transform
      drop_na(ln_lifespan_c, ln_Genome_Size_c, rate_raw) %>%
      filter(is.finite(ln_lifespan_c), is.finite(ln_Genome_Size_c), is.finite(rate_raw))
    ...
    ## Model: raw rate as response
    formula_str <- "rate_raw ~ ln_lifespan_c * ln_Genome_Size_c"
...
if (length(pgls_results_all_regions) > 0) {
  ...
  out_file <- "pgls_results_8_regions_genome10_intergenic10_others16_dinuc_RATE_LS_CENTERED_G_CENTERED_interaction_raw_freq_as_response.csv"
