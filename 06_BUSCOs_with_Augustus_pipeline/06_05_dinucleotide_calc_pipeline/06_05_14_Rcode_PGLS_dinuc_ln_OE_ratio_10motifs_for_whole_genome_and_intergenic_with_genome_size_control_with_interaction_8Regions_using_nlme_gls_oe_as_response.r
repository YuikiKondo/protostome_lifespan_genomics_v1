...
for (region in regions) {
  ...
  for (dinuc in dinuc_columns_to_use) {
    ...
    ## ---- FIT PGLS (try lambda start=1, fallback to 0.5) ----
    pgls_model <- tryCatch(
      {
        gls(
      ...
      error = function(e1) {
        message("    Default fit failed (lambda start=1) for ", dinuc, " in ", region,
                ". Retrying with lambda start=0.5. Reason: ", e1$message)
        ...
        tryCatch(
          {
            gls(
              as.formula(formula_str),
              data = region_df,
              correlation = corPagel(0.5, phy = phylo_tree, form = ~species_name),
              method = "ML"
            )
          },
          error = function(e2) {
            message("    Fallback fit also failed (lambda start=0.5) for ", dinuc, " in ", region,
                    ". Skipping. Reason: ", e2$message)
            return(NULL)
          }
        )
      }
