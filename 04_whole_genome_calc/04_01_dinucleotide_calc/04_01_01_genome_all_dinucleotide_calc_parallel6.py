...
def process_all_fasta_files(input_dir, output_dir, num_processes, max_files=100):
    ...
    output_file = os.path.join(output_dir, "genome_dinucleotide_summary500_650.txt")

    # Ensure header row is written before processing
    write_header(output_file)

    fasta_files = [os.path.join(input_dir, f) for f in os.listdir(input_dir) if f.endswith(".fna")][500:650]
