...
# Define file paths
input_file <- "/home/y-kondo/protostome_lifespan_model/BUSCO_seq_directory/TSS_Github_check/all_species_sliding_window_dinucleotide_precise_with_species.csv"
output_dir <- "/home/y-kondo/protostome_lifespan_model/BUSCO_seq_directory/TSS_Github_check/Dinucleotide_rate_plots_precise"
...
# Generate one PNG file per dinucleotide rate index
for (index in dinucleotide_rates) {
  ...
  for (assembly_name in assemblies) {
    ...
    p <- ggplot(subset_df, aes(x = Position, y = .data[[index]])) +
      ...
      geom_vline(xintercept = -300, lty = 2, colour = "grey10", linewidth = 0.5) +
