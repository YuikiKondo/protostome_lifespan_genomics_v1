...
## ---- Define dinuc OE columns to use (collapsed in ALL regions) ----
...
if (length(dinuc_oe_collapsed_10) == 0) {
  stop("No collapsed dinucleotide OE columns found. Check that *_and_*_OE and AT/TA/CG/GC_OE exist.")
}
...
for (region in regions) {
  ...
  dinuc_columns_to_use <- dinuc_oe_collapsed_10
...
if (length(pgls_results_all_regions) > 0) {
  ...
  out_file <- "pgls_results_3_regions_genome_intergenic_10_others16_dinuc_ln_OE_LS_CENTERED_G_CENTERED_interaction_OE_as_response.csv"
